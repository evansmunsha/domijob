...
export const GUEST_CREDIT_COOKIE = "domijob_guest_credits"
export const MAX_GUEST_CREDITS = 50

export async function handleCreditCharge(featureType: string) {
  ...
  if (userId) {
    ...
    if (userCredits < creditCost) throw new Error("Insufficient credits.")
    await deductCredits(userId, featureType)
    return { userId, isGuest: false, creditsUsed: creditCost, remainingCredits: userCredits - creditCost }
  }
  ...
  return {
    ...
    guestCreditsToSet: updatedCredits, // include this to be used in API handler
  }
}
